...
function updateSiswaAkun($nisn, $alamat, $no_telp, $password = null)
{
    $conn = getDatabaseConnection();

    if ($password !== null) {
        $stmt = $conn->prepare("UPDATE siswa SET alamat = ?, no_telp = ?, password = ? WHERE nisn = ?");
        return $stmt->execute([$alamat, $no_telp, $password, $nisn]);
        exit;
    } else {
        $stmt = $conn->prepare("UPDATE siswa SET alamat = ?, no_telp = ? WHERE nisn = ?");
        return $stmt->execute([$alamat, $no_telp, $nisn]);
        exit;
    }
}
